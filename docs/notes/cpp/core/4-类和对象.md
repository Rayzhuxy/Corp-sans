...
//&p，（int） &p，（int*） &p有什么区别
// &p 返回 Person* 类型（指向 Person 对象的指针）
// (int)&p 将指针地址 强制转换为 int 类型（整数）,但是指针大小是 8 字节（64 位），而 int 通常是 4 字节（32 位），会因为丢失精度而报错
// (int*)&p 将 Person* 强制转换为 int*（指向 int 的指针），即返回类型为 int* ，但是内容与&p一样
